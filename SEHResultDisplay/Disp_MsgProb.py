...
TEST_SET = pickle.load(open("../rawresults/MsgProb/TEST_SET","r"))
x_axis = pickle.load(open("../rawresults/MsgProb/TEST_RANGE","r"))
# TEST_SET = ['MDP', 'CAS', 'GREEDY', 'RANDOM', 'THRESHOLDRANDOM']

exp_v, steady_v, steady_chg_r, steady_snd_r = [],[],[],[]
for j,t in enumerate(TEST_SET): 
    exp_v.append(pickle.load(open("../rawresults/MsgProb/exp_value_lis_"+'_'+t,"r")))
    steady_v.append(pickle.load(open("../rawresults/MsgProb/steady_value_lis"+'_'+t,"r")))
    steady_chg_r.append(pickle.load(open("../rawresults/MsgProb/steady_chg_rate_lis_"+'_'+t,"r")))
    steady_snd_r.append(pickle.load(open("../rawresults/MsgProb/steady_snd_rate_lis_"+'_'+t,"r")))
...
xlabel('Messenger-sink contact prob. pattern',fontsize=14)
...
pp = PdfPages('MsgProb_figure1.pdf')
...
plt.figure(figsize=(4.5,5.0))
grid(True, which="both")
plot(x_axis,steady_chg_r[0],color='red',markerfacecolor='none', markeredgecolor='red', marker='o',markersize=8,label='MDP', linestyle='--')
plot(x_axis,steady_chg_r[1],color='green',markerfacecolor='none', markeredgecolor='green', marker='^',markersize=8,label='CAS', linestyle='--')
plot(x_axis,steady_chg_r[2],color='blue',markerfacecolor='none', markeredgecolor='blue', marker='s',markersize=8,label='GRD', linestyle='--')
...
locs, labels = plt.yticks()
plt.setp(labels, rotation=90)
pp = PdfPages('MsgProb_figure3.pdf')
plt.savefig(pp, format='pdf')
pp.close()
...
# locs, labels = plt.yticks()
# plt.setp(labels, rotation=90)
# pp = PdfPages('MsgProb_figure4.pdf')
# plt.savefig(pp, format='pdf')
# pp.close()