...
TEST_SET = pickle.load(open("../rawresults/UpdatePrecs/TEST_SET","r"))
x_axis = pickle.load(open("../rawresults/UpdatePrecs/TEST_RANGE","r"))
# TEST_SET = ['GREEDY', 'RANDOM', 'THRESHOLDRANDOM']

exp_v, steady_v, steady_chg_r, steady_snd_r = [],[],[],[]
time_con = []
for j,t in enumerate(TEST_SET): 
    exp_v.append(pickle.load(open("../rawresults/UpdatePrecs/exp_value_lis_"+'_'+t,"r")))
    steady_v.append(pickle.load(open("../rawresults/UpdatePrecs/steady_value_lis"+'_'+t,"r")))
    steady_chg_r.append(pickle.load(open("../rawresults/UpdatePrecs/steady_chg_rate_lis_"+'_'+t,"r")))
    steady_snd_r.append(pickle.load(open("../rawresults/UpdatePrecs/steady_snd_rate_lis_"+'_'+t,"r")))
    time_con.append(pickle.load(open("../rawresults/UpdatePrecs/time_consumed_lis"+'_'+t,"r")))
...
pp = PdfPages('UpdatePrecs_figure1.pdf')
...
locs, labels = plt.yticks()
plt.setp(labels, rotation=90)
pp = PdfPages('UpdatePrecs_figure3.pdf')
plt.savefig(pp, format='pdf')
pp.close()
...
locs, labels = plt.yticks()
plt.setp(labels, rotation=90)
pp = PdfPages('UpdatePrecs_figure4.pdf')
plt.savefig(pp, format='pdf')
pp.close()
...
xlabel('Update precision',fontsize=14)
ylabel('Time consumed',fontsize=16)
subplots_adjust(top=0.93,bottom=0.16,left=0.12, right=0.95)
# legend(loc='best', ncol=1,fancybox=True,shadow=True)
legend(loc='best',fancybox=True)
locs, labels = plt.yticks()
plt.setp(labels, rotation=90)
pp = PdfPages('UpdatePrecs_figure_time.pdf')
plt.savefig(pp, format='pdf')
pp.close()